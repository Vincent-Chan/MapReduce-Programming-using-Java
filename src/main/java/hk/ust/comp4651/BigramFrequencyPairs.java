...
public class BigramFrequencyPairs extends Configured implements Tool {
	...
	private static class MyMapper extends
		...
		public void map(LongWritable key, Text value, Context context)
			...
			for (int i = 0; i < words.length - 1; ++i)
			{
				...
				// Case A: the bigram is a word follow by a null character
				BIGRAM.set(words[i], "") ;
				context.write(BIGRAM, ONE) ;

				// Case B: the bigram is a word follow by a word after current one
	...
	private static class MyReducer extends
		...
		public void reduce(PairOfStrings key, Iterable<IntWritable> values,
			...
			// Initialize a sum counter as 0 in the beginning
			int s = 0 ;

			// Use a for loop to loop through all the values
			for (IntWritable v: values)
			{
				// Summation the sum counter
				s += v.get() ;
			}
