...
public class CORPairs extends Configured implements Tool {
	...
	public static class CORPairsMapper2 extends Mapper<LongWritable, Text, PairOfStrings, IntWritable> {
		...
		protected void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
			...
			String left_token = new String() ;
			String right_token = new String() ;
			IntWritable ONE = new IntWritable(1) ;

			while (doc_tokenizer.hasMoreTokens() == true)
			{
				word_token = doc_tokenizer.nextToken() ;
				word_set_sorted.add(word_token) ;
			}
			...
			for (int i = 0; i < word_list.length; ++i)
			{
				...
				{
					left_token = word_list[i] ;
					right_token = word_list[j] ;

					KEY.set(left_token, right_token) ;
					context.write(KEY, ONE) ;
				}
			}
	...
	public static class CORPairsReducer2 extends Reducer<PairOfStrings, IntWritable, PairOfStrings, DoubleWritable> {
		...
		protected void reduce(PairOfStrings key, Iterable<IntWritable> values, Context context) throws IOException, InterruptedException {
			...
			int left_and_right_freq = 0 ;
			double cor_value ;

			for (IntWritable v: values)
			{
				left_and_right_freq += v.get() ;
			}

			double left_token_freq = word_total_map.get(key.getLeftElement().toString()) ;
			double right_token_freq = word_total_map.get(key.getRightElement().toString()) ;

			cor_value = (double) left_and_right_freq / (left_token_freq * right_token_freq) ;
			...
			double left_freq = word_total_map.get(key.getLeftElement().toString()) ;
			double right_freq = word_total_map.get(key.getRightElement().toString()) ;

			cor_value = s / (left_freq * right_freq) ;
